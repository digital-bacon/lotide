...
const flatten = (array) => {
  // Edge case: We need a way to check if the item is an array before flattening it
  // Add a buffer for the result
  const buffer = [];
  ...
  for (const item of array) {
    ...
    if (Array.isArray(item)) {
      // Loop through an existing array of items
      for (const nestedArrayItem of item) {
        //Insert this element at the end of the buffer
        buffer.push(nestedArrayItem);
      }
    } else {
      // Insert this element at the end of the buffer
      buffer.push(item);
